...
def vrcnn_predict_on_test_set(vrcnn_model, test_ds):
    ...
    for img_idx in range(total_images):
        ...
        rois = torch.tensor(test_ds.rois[img_idx]).to(device)
        pred_classes = []
        pred_deltas = []

        for roi_idx in range(len(rois)):
            # Get the roi_crop
            crop_roi = test_ds.rois[img_idx][roi_idx]
            ...
            roi_classes = test_ds.roi_classes[img_idx][roi_idx]
            roi_deltas = test_ds.roi_deltas[img_idx][roi_idx]
            # Pre-process data
            processed = test_ds.collate_fn([(roi_crop, roi_classes, roi_deltas)])
            roi_crop, roi_class, roi_delta = processed
            with torch.no_grad():
                # Get predictions
                vrcnn_model.eval()
                _pred_classes, _pred_deltas = vrcnn_model(roi_crop)
                pred_classes.append(_pred_classes)
                pred_deltas.append(_pred_deltas)

        # Concatenate the results
        pred_classes = torch.concat(pred_classes)
        pred_deltas = torch.concat(pred_deltas)
